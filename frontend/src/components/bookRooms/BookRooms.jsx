...
const BookRooms = ({ setOpenRooms, hotelId }) => {
  const { data, loading, error } = useFetch(`https://notbooking.onrender.com/api/hotels/room/${hotelId}`);
    ...
    try {
      await Promise.all(
        selectedRooms.map((roomId) => {
          const res = axios.put(`https://notbooking.onrender.com/api/rooms/availability/${roomId}`, {
            dates: dateRange,
          });
          return res.data;
        })
      ...
      await axios.put(`https://notbooking.onrender.com/api/users/reserved/${userId}`, {
        roomNumber: selectedRooms,
      });

      const updatedUser = await axios.get(`https://notbooking.onrender.com/api/users/${userId}`);
